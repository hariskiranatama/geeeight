...
class MeioUploadBehavior extends ModelBehavior {
	...
	function beforeSave(&$model) {
		foreach($this->__fields[$model->alias] as $fieldName=>$options){
			...
			if (count($options['allowed_ext']) > 0 && in_array($model->data[$model->alias][$fieldName]['type'], array('image/jpeg', 'image/pjpeg', 'image/gif', 'image/png'))) {
    ...
    function resizeOnly($source_image, $new_image, $resize_width, $resize_height){
        //CakeLog::write('debug', 'in resizeOnly');
        //CakeLog::write('debug', '$source_image='.$source_image);
        //CakeLog::write('debug', '$new_image='.$new_image);
        //CakeLog::write('debug', '$resize_width='.$resize_width);
        //CakeLog::write('debug', '$resize_height='.$resize_height);
        //default value
        $result = false;
        //sanitize
        $resize_width = intval($resize_width);
