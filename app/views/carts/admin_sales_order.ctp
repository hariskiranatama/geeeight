...
<h2><?php  __d("admin", 'Sales Order');?></h2>
...
<h3><?php __d("admin", 'Shopping Cart'); ?></h3>
    ...
    <fieldset>
        <legend><?php __d("admin", 'Add Shipping Costs'); ?></legend>
        <?php echo $this->Form->input('id', array('type' => 'hidden', 'value' => $cart['Cart']['id'])); ?>
        <?php echo $this->Form->input('shipping_costs'); ?>
    </fieldset>
    <?php echo $this->Form->submit(__d("admin", 'Submit', true), array('after' => $this->Html->link('Cancel', array('controller' => 'carts', 'action' => 'index', 'checkout')))); ?>
