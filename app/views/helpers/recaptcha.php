...
class RecaptchaHelper extends AppHelper {
    ...
    function display_form($output_method = 'return', $error = null, $use_ssl = false){
        $data = $this->__form_ajax(Configure::read("Recaptcha.pubKey"),$error,$use_ssl);
     ...
     * @return string - The HTML to be embedded in the user's form.
     */
    function __form_ajax($pubkey, $error = null, $use_ssl = false){
        if ($pubkey == null || $pubkey == '') {
            die ("To use reCAPTCHA you must get an API key from <a href='http://recaptcha.net/api/getkey'>http://recaptcha.net/api/getkey</a>");
        }
        ...
        return '<div id="recaptcha_div"><img src="'.$this->webroot.'images/loading.gif" alt="" /> Loading ReCAPTCHA, please wait...  </div>
